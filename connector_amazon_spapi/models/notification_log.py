...
class AmazonNotificationLog(models.Model):
    ...
    def _compute_display_name(self):
        for record in self:
            if record.notification_type and record.message_id:
                record.display_name = f"{record.notification_type} ({record.message_id[:8]}...)"
    ...
    def process_notification(self):
        ...
        if self.state not in ("received", "error"):
            _logger.info(
                "Skipping notification %s in state %s", self.id, self.state
            )
            return
    ...
    def _handle_order_change(self):
        ...
        _logger.info("Processing ORDER_CHANGE for order %s", amazon_order_id)

        # Find or sync the order
        order_binding = self.env["amazon.sale.order"].search([
        ...
        if order_binding:
            ...
            if shop:
                with self.backend_id.work_on("amazon.sale.order") as work:
                    ...
                    result = adapter.get_order(amazon_order_id)

                if result and result.get("payload"):
                    order_data = result["payload"]
                    new_order = self.env["amazon.sale.order"]._create_or_update_from_amazon(
                        shop, order_data
                    )
    ...
    def _handle_listings_change(self):
        ...
        if not seller_sku:
            _logger.warning("LISTINGS_ITEM_STATUS_CHANGE missing SellerSKU: %s", payload)
            return
        ...
        if binding:
            ...
            if product:
                ...
                if marketplace:
                    new_binding = self.env["amazon.product.binding"].create({
                        "backend_id": self.backend_id.id,
    ...
    def action_view_raw_message(self):
        ...
        return {
            ...
            "view_mode": "form",
            "target": "new",
            "context": {"form_view_ref": "connector_amazon_spapi.view_notification_log_raw_form"},
        }
