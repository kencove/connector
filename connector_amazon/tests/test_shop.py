...
class TestAmazonShop(common.CommonConnectorAmazonSpapi):
    ...
    def test_sync_orders_fetches_from_api(self, mock_call_sp_api):
        ...
        order = self.env["amz.sale.order"].search(
            [
    ...
    def test_sync_orders_creates_order_bindings(self, mock_call_sp_api):
        """Test sync_orders creates amz.sale.order bindings"""
        ...
        orders = self.env["amz.sale.order"].search([("shop_id", "=", self.shop.id)])
    ...
    def test_sync_orders_updates_existing_orders(self, mock_call_sp_api):
        ...
        existing_order = self.env["amz.sale.order"].create(
            {
                "shop_id": self.shop.id,
    ...
    def test_multiple_shops_same_backend(self):
        ...
        marketplace2 = self.env["amz.marketplace"].create(
            {
                "name": "Amazon.co.uk",
    ...
    def test_push_stock_creates_feed(self):
        ...
        # Call push_stock
        with mock.patch.object(type(self.env["amz.feed"]), "with_delay") as m:
    ...
    def test_cron_push_stock_hourly(self):
        ...
        # Create hourly shop
        hourly_shop = self.env["amz.shop"].create(
            {
    ...
    def test_cron_push_stock_skips_inactive_shops(self):
        ...
        inactive_shop = self.env["amz.shop"].create(
            {
                "name": "Inactive Shop",
                "backend_id": self.backend.id,
                "marketplace_id": self.marketplace.id,
    ...
    def test_cron_push_shipments(self):
        ...
        order = self.env["amz.sale.order"].create(
            {
    ...
    def test_cron_push_stock_respects_interval_settings(self):
        ...
        with mock.patch.object(
            type(self.env["amz.shop"]), "action_push_stock"
        ) as mock_push:
            self.env["amz.shop"].cron_push_stock()
