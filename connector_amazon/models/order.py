...
class AmazonSaleOrder(models.Model):
    ...
    shop_id = fields.Many2one(comodel_name="amz.shop", ondelete="set null")
    marketplace_id = fields.Many2one(
        comodel_name="amz.marketplace", ondelete="set null"
    )
    ...
    def _build_fulfillment_feed_xml(self, picking, carrier_name, tracking_ref):
        ...
        for move in picking.move_ids.filtered(lambda m: m.state == "done"):
            ...
            if order_line:
                ...
                amazon_line = self.env["amz.sale.order.line"].search(
                    [
    ...
    def _create_or_update_from_amazon(self, shop, amazon_order):  # noqa: C901
        ...
        amz_order_id = amazon_order.get("AmazonOrderId")

        # Skip orders already imported by the EE sale_amazon module to
        # avoid duplicates when both modules are installed.
        if shop.backend_id.sale_amazon_installed:
            if "amazon_order_ref" in self.env["sale.order"]._fields:
                existing_ee = self.env["sale.order"].search(
                    [("amazon_order_ref", "=", amz_order_id)],
                    limit=1,
                )
                if existing_ee:
                    _logger.info(
                        "Skipping order %s — already imported by sale_amazon (SO %s)",
                        amz_order_id,
                        existing_ee.name,
                    )
                    return self.browse()
        ...
        if config["test_enable"] and not self.env.context.get(
            "amz_sync_lines_in_tests"
        ):
            is_mocked = hasattr(shop.backend_id._call_sp_api, "assert_called")
            if not is_mocked:
                should_sync_lines = False

        if should_sync_lines and not self.env.context.get("amz_skip_line_sync"):
            self._sync_order_lines(binding, shop, amz_order_id)
    ...
    def _sync_order_lines(self, binding=None, shop=None, amz_order_id=None):
        ...
        if binding:
            ...
            amz_order_id = amz_order_id or binding.external_id
        else:
            self.ensure_one()
            binding = self
            shop = shop or self.shop_id
            amz_order_id = amz_order_id or self.external_id

        line_model = self.env["amz.sale.order.line"]
        ...
        if config["test_enable"]:
            ...
            if not backend_mocked and not adapter_mocked:
                if not self.env.context.get("amz_allow_orderitem_api"):
                    return
...
class AmazonSaleOrderLine(models.Model):
    ...
    def _create_or_update_from_amazon(self, amazon_order_binding, shop, amazon_item):
        ...
        binding = self.search(
            [
                ("amz_order_id", "=", amazon_order_binding.id),
                ("external_id", "=", item_id),
            ],
            limit=1,
        )
    ...
    def _get_product_by_sku(self, shop, seller_sku):
        ...
        Searches amz.product.binding first, then falls back to
        product.product by default_code.

        Returns:
            tuple: (amz.product.binding or empty recordset, product.product or empty recordset)
        """
        ...
        return self.env["amz.product.binding"], product
